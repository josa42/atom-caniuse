...
export default {
  ...
  activate(/* state */) {
    atom.commands.add('atom-workspace', {
      'can-i-use:show': () => {
        if (!this.caniuseView) {
          const CaniuseView = require('./caniuse-view')
          this.caniuseView = new CaniuseView()
        }
        this.caniuseView.show()
      },
      ...
      'can-i-use:update': () => {
        ...
        loadData()
          .then(() => {
            if (this.caniuseView) {
              this.caniuseView.populate()
            }
          })
          .catch(() => {
            if (this.caniuseView) {
              this.caniuseView.setError('Loading data failed!')
            }
          })
      }
    })
